...
<p align="center"><sub>Figure 1 — 상대 거리를 directed edge representation으로 표현하는 방식</sub></p>
...
```math
\begin{aligned}
\mathrm{score}_{\mathrm{content}}&=QK^\top,\\
\mathrm{shape}(\mathrm{score}_{\mathrm{content}})&=[B,H,N,N].
\end{aligned}
```
...
```math
\mathrm{score}_{\mathrm{relative}}[b,h,i,j]
=\mathrm{dot}(Q[b,h,i,:],A_K[i,j,:])
```

두 score를 더하고 scaling한다.

```math
E=\frac{\mathrm{score}_{\mathrm{content}}
+\mathrm{score}_{\mathrm{relative}}}{\sqrt{d_z}}
```
