...
![DETR의 image memory·object query·Hungarian set loss](https://github.com/user-attachments/assets/799b496a-c1dd-49f5-9ccf-6925b296874c)
...
```text
sigma_hat = argmin_{sigma in S_N}
            sum_i L_match(y_i, y_hat_{sigma(i)})
```

Object ground truth의 pairwise matching cost는 class와 box를 함께 본다.

```text
L_match = - p_hat_{sigma(i)}(c_i)
          + 1[c_i != empty] L_box(b_i, b_hat_{sigma(i)})
```
...
```text
L = sum_i [
    - log p_hat_{sigma_hat(i)}(c_i)
    + 1[c_i != empty] L_box(b_i, b_hat_{sigma_hat(i)})
]
```
...
```text
L_box = lambda_iou L_GIoU(b,b_hat)
        + lambda_L1 ||b-b_hat||_1
```
...
```text
L_total = L_final + sum_{l=1}^{L_dec-1} L_aux^l
```
...
```text
decoder self-attention : O(N²d)
cross-attention        : O(NSd)
```
