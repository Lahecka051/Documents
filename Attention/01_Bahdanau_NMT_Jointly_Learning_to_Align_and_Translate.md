...
```text
e_(i,j) = alignment_score(s_(i-1), h_j)
...
```text
forward_h_j  = forward_RNN(x_j, forward_h_(j-1))
backward_h_j = backward_RNN(x_j, backward_h_(j+1))
```

두 state를 concatenate해 annotation을 만든다.

```text
h_j = concat(forward_h_j, backward_h_j)
```
...
```text
e_(i,j)
= v_a^T tanh(W_s s_(i-1) + W_h h_j + b_a)
```

Decoder state와 encoder annotation을 각각 선형 변환한 뒤 더하고, `tanh`와 learned vector `v_a`로 scalar score를 만든다.
...
```text
projected_state = s_(i-1) W_s
shape: [B, d_a]

projected_memory = H W_h
shape: [B, Tx, d_a]
```
...
```text
alpha_i = softmax(e_i, dim=source_position)
shape: [B, Tx]
...
```text
alpha = [0.11, 0.18, 0.71]
```

Context는 다음 weighted sum이다.

```text
c_i = 0.11 h_the + 0.18 h_red + 0.71 h_car
```
...
Target length가 `Ty`, source length가 `Tx`라면 모든 target-source pair에 score를 계산한다.

```text
alignment score complexity: O(Ty * Tx * d_a)
alignment matrix memory:    O(Ty * Tx) if stored
```
...
Target step마다 모든 source annotation을 scoring하므로 `O(Tx Ty)` pair를 계산한다. 긴 sequence에서는 비용이 커진다.
...
```text
e_(i,j) = v_a^T tanh(W_s s_(i-1) + W_h h_j)
alpha_(i,:) = softmax(e_(i,:))
c_i = sum_j alpha_(i,j) h_j
```
