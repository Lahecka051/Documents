...
\qquad i\in\lbrace1,2,3,4\rbrace.
```
...
```math
\hat F_i=\mathrm{Linear}_{C_i\rightarrow C}(F_i).
```
...
```math
\begin{aligned}
\tilde F_i&=\mathrm{Upsample}_{H/4,W/4}(\hat F_i),\\
F&=\mathrm{Linear}_{4C\rightarrow C}
(\mathrm{Concat}(\tilde F_1,\ldots,\tilde F_4)),\\
M&=\mathrm{Linear}_{C\rightarrow N_{cls}}(F).
\end{aligned}
```
