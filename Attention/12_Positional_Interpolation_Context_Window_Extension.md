...
- 원본 파일: `C:\Users\lkm\Documents\Codex\Embbeded_OnDevice_ComputerVision\attention_papers_pdf\12_Positional_Interpolation_Context_Window_Extension.pdf`
- 제목: Extending Context Window of Large Language Models via Position Interpolation
- 저자: Shouyuan Chen et al.
- 발표: 2023
- 주제: Position Interpolation의 context 확장 이론
- 핵심 키워드: Position Interpolation, RoPE scaling, context window extension

## 한눈에 보는 요약

Position Interpolation은 RoPE 기반 LLM의 context window를 확장하기 위해 긴 위치 index를 기존 학습 범위 안으로 압축한다.

예를 들어 2048 길이로 학습한 모델을 4096 길이로 쓰고 싶다면 위치 index를 절반으로 나누어 RoPE에 넣는다.

Extrapolation이 아니라 interpolation으로 바꾸기 때문에 fine-tuning이 훨씬 안정적이다.

## 연구 배경과 문제의식

RoPE 모델은 학습 context length보다 큰 position index에서 성능이 급격히 나빠질 수 있다.

RoPE position index를 목표 길이 비율로 압축해 extrapolation을 interpolation 문제로 바꾸는 방법이다.

직접 extrapolation은 모델이 보지 못한 회전 각도와 frequency 조합을 사용하게 만든다.

## 이 논문에서 먼저 잡아야 할 이론적 축

이 논문의 중심축은 `Position Interpolation의 context 확장 이론`이다.
...
### Position Interpolation의 RoPE 확장 관점
...
```text
기존 RoPE는 위치 `p`에 대해 angle `p * theta_i`를 사용한다.
```
...
```text
Position Interpolation은 angle을 `(p / s) * theta_i`로 바꾼다.
```
...
```text
따라서 긴 context의 마지막 위치 `L'`도 RoPE 관점에서는 기존 최대 위치 `L` 근처로 들어온다.
```
...
```text
짧은 fine-tuning 동안 모델은 압축된 거리 체계에 적응한다.
```
...
```text
Attention score의 상대 위치 차이도 `(p_i - p_j) / s`로 축소되어 학습 범위 밖 회전을 피한다.
```
...
- 위치 resolution이 줄어들어 아주 세밀한 거리 구분이 어려워질 수 있다.
...
- 위치 resolution이 줄어들어 아주 세밀한 거리 구분이 어려워질 수 있다.
- 확장 배율이 커질수록 압축이 심해져 성능 저하가 커질 수 있다.
- RoPE 위치 문제를 다루지만 attention 계산량과 KV cache 메모리 증가는 해결하지 않는다.

## 후속 논문과의 연결점

YaRN과 LongRoPE는 Position Interpolation의 단순 uniform scaling을 더 정교하게 만들어 주파수별, 구간별 scaling을 적용한다.

## 개인 학습/연구 메모

Position Interpolation은 `긴 위치를 새로 외삽하지 말고, 기존 위치 구간 안에 압축해서 넣자`는 아이디어다.
