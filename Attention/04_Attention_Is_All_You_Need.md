...
```math
h_t=\operatorname{RNN}(h_{t-1},x_t)
```
...
![세 attention 유형과 scaled dot-product attention 계산](https://github.com/user-attachments/assets/b4a89e7a-98af-4b9b-848e-b69386679ba0)
...
- $\operatorname{softmax}(\cdots)$: 각 query가 key들을 얼마나 볼지 나타내는 attention weight
- $\operatorname{softmax}(\cdots)V$: attention weight로 value를 weighted sum한 결과

한 query vector $q$와 여러 key/value가 있을 때는 다음처럼 볼 수 있다.

```math
\begin{aligned}
\operatorname{score}_j &= \frac{q\cdot k_j}{\sqrt{d_k}},\\
\alpha_j &= \frac{\exp(\operatorname{score}_j)}
{\sum_l\exp(\operatorname{score}_l)},\\
\operatorname{output} &= \sum_j\alpha_jv_j.
\end{aligned}
```
...
H+\operatorname{MultiHeadSelfAttention}(H)\right),\\
...
\begin{aligned}
T' &=
\operatorname{LayerNorm}\!\left(T+\operatorname{MaskedSelfAttention}(T)\right),\\
T'' &=
\operatorname{LayerNorm}\!\left(
T'+\operatorname{CrossAttention}(\operatorname{query}=T',
\operatorname{key}=Z,\operatorname{value}=Z)\right),\\
T''' &=
\operatorname{LayerNorm}\!\left(T''+\operatorname{FFN}(T'')\right).
\end{aligned}
```
...
K &= ZW_K,\\
V &= ZW_V,\\
\operatorname{CrossAttention}(T,Z)
&=\operatorname{softmax}\!\left(\frac{QK^\top}{\sqrt{d_k}}\right)V.
\end{aligned}
```
...
```math
\begin{aligned}
\operatorname{shape}(W_Q)&=[d_{\mathrm{model}},d_k],\\
\operatorname{shape}(W_K)&=[d_{\mathrm{model}},d_k],\\
\operatorname{shape}(W_V)&=[d_{\mathrm{model}},d_v].
\end{aligned}
```
...
\begin{aligned}
Q&=HW_Q,
&\operatorname{shape}(Q)&=[B,n,d_k],\\
K&=HW_K,
&\operatorname{shape}(K)&=[B,n,d_k],\\
V&=HW_V,
&\operatorname{shape}(V)&=[B,n,d_v].
\end{aligned}
```
...
```math
\begin{aligned}
\operatorname{score}(\mathrm{love}\to\mathrm{I})&=0.2,\\
\operatorname{score}(\mathrm{love}\to\mathrm{love})&=1.5,\\
\operatorname{score}(\mathrm{love}\to\mathrm{NLP})&=1.1.
\end{aligned}
```
...
```math
\operatorname{new}_{\mathrm{love}}
...
![QKV projection과 multi-head attention 결합](https://github.com/user-attachments/assets/9aa31aab-1004-4858-9cb9-b181d78ff9c3)

그림의 (a)는 head $i$에서 $Q$, $K$, $V$에 서로 다른 learned linear projection을 적용하는 과정이고, (b)는 $h$개 head를 concatenate한 뒤 $W^O$로 다시 $d_{\mathrm{model}}$ 차원에 projection하는 과정이다. 그림의 함수와 parameter 표기는 원논문 Section 3.2.2를 따른다.
...
![Position-wise feed-forward network의 행렬 계산과 position-wise 적용](https://github.com/user-attachments/assets/228f1b31-4f0c-43ca-97b7-b47b64382395)

그림의 (a)는 sequence 전체를 행렬로 계산할 때 feature dimension이 `512 -> 2048 -> 512`로 변하는 과정이고, (b)는 동일한 `W_1`, `b_1`, `W_2`, `b_2`가 각 position에 서로 독립적으로 적용되는 과정을 나타낸다. FFN에서는 attention처럼 서로 다른 position 사이의 값을 곱하거나 가중합하지 않는다.
...
```math
\operatorname{input\ representation}
=\operatorname{token\ embedding}+\operatorname{positional\ encoding}
```
...
```math
\begin{aligned}
\operatorname{shape}(X)&=[n,d_{\mathrm{model}}],\\
\operatorname{shape}(\operatorname{PE})&=[n,d_{\mathrm{model}}],\\
H&=X+\operatorname{PE},\\
\operatorname{shape}(H)&=[n,d_{\mathrm{model}}].
\end{aligned}
```

여기서 $H=X+\operatorname{PE}$는 계산 구조를 설명하기 위한 표기다. 원논문은 이 관계를 별도의 기호 $H$로 정의하지 않고, positional encodings를 input embeddings에 더한다고 서술한다.
...
```math
\begin{aligned}
\operatorname{PE}_{(\mathrm{pos},2i)}
&=\sin\!\left(\frac{\mathrm{pos}}
{10000^{2i/d_{\mathrm{model}}}}\right),\\
\operatorname{PE}_{(\mathrm{pos},2i+1)}
&=\cos\!\left(\frac{\mathrm{pos}}
{10000^{2i/d_{\mathrm{model}}}}\right).
\end{aligned}
```

![Sinusoidal positional encoding의 정의와 차원 배치](https://github.com/user-attachments/assets/3dc29f65-d719-48d9-b593-f087a265b3d7)

그림의 (a)는 embedding과 positional encoding의 원소별 덧셈, (b)는 Section 3.5의 두 함수, (c)는 $d_{\mathrm{model}}=512$일 때 $2i$와 $2i+1$이 차원에 배치되는 방법을 나타낸다.
...
| $\operatorname{PE}_{(\mathrm{pos},2i)}$ | position $\mathrm{pos}$의 짝수 dimension 값 | $[-1,1]$ |
| $\operatorname{PE}_{(\mathrm{pos},2i+1)}$ | position $\mathrm{pos}$의 홀수 dimension 값 | $[-1,1]$ |
| $\operatorname{PE}_{\mathrm{pos}}$ | position $\mathrm{pos}$에서의 positional encoding vector 전체 | $[512]$ |
| $\operatorname{PE}$ | 모든 position의 positional encoding을 쌓은 행렬 | $[n,512]$ |
...
\begin{aligned}
10000^{2\cdot0/512} &= 1,\\
\operatorname{PE}_{(1,0)} &= \sin(1/1)=0.841471,\\
\operatorname{PE}_{(1,1)} &= \cos(1/1)=0.540302.
\end{aligned}
```
...
\operatorname{PE}_{(1,2)}
&=\sin\!\left(\frac{1}{10000^{2/512}}\right)=0.821856,\\
\operatorname{PE}_{(1,3)}
...
| $\mathrm{pos}$ | $\operatorname{PE}_{(\mathrm{pos},0)}$ | $\operatorname{PE}_{(\mathrm{pos},1)}$ | $\operatorname{PE}_{(\mathrm{pos},2)}$ | $\operatorname{PE}_{(\mathrm{pos},3)}$ | $\operatorname{PE}_{(\mathrm{pos},4)}$ | $\operatorname{PE}_{(\mathrm{pos},5)}$ | $\operatorname{PE}_{(\mathrm{pos},6)}$ | $\operatorname{PE}_{(\mathrm{pos},7)}$ |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|
...
![Sinusoidal positional encoding의 전체 행렬과 주파수별 곡선](https://github.com/user-attachments/assets/278f3e3e-b90a-4ef8-860d-30ca3a06d832)
...
논문이 제시하는 핵심 가설은 고정된 offset $k$에 대해 $\operatorname{PE}_{(\mathrm{pos}+k)}$를 $\operatorname{PE}_{\mathrm{pos}}$의 선형함수로 표현할 수 있다는 것이다.
...
$x+\operatorname{Sublayer}(x)$가 residual connection이다. 이는 입력을 그대로 다음 단계로 보낼 수 있는 경로를 만든다.
...
```math
X=\operatorname{Embedding}(\operatorname{source\_ids})\sqrt{d_{\mathrm{model}}}
```

이후 positional encoding을 더한다.

```math
H_0=X+\operatorname{PE}
```
...
\begin{aligned}
\operatorname{head}_i
&=\operatorname{Attention}(HW_i^Q,HW_i^K,HW_i^V),\\
\operatorname{MHA}
...
```math
H_{\mathrm{attn}}
=\operatorname{LayerNorm}\!\left(H+\operatorname{Dropout}(\operatorname{MHA})\right)
```
...
```math
\begin{aligned}
F&=\operatorname{ReLU}(H_{\mathrm{attn}}W_1+b_1)W_2+b_2,\\
H_{\mathrm{next}}
&=\operatorname{LayerNorm}\!\left(H_{\mathrm{attn}}+\operatorname{Dropout}(F)\right).
\end{aligned}
```
...
```math
\begin{aligned}
Z&=\operatorname{Encoder}(\operatorname{source}),\\
\operatorname{shape}(Z)&=[B,n,d_{\mathrm{model}}].
\end{aligned}
```
...
```math
\begin{aligned}
Q&=\operatorname{decoder\_state}W_Q,\\
K&=\operatorname{encoder\_output}W_K,\\
V&=\operatorname{encoder\_output}W_V,\\
...
```math
D_{\mathrm{next}}
=\operatorname{LayerNorm}\!\left(D_{\mathrm{cross}}+\operatorname{FFN}(D_{\mathrm{cross}})\right)
```
...
```math
\begin{aligned}
\operatorname{logits}&=D_{\mathrm{final}}W_{\mathrm{vocab}}^\top+b,\\
...
```math
\operatorname{shape}(\operatorname{probs})=[B,m,\operatorname{vocab\_size}]
```
...
$\operatorname{warmup\_steps}=4000$이다.
