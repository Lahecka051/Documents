...
<p align="center"><sub>Figure 2 패널 재배치 — MHA·GQA·MQA의 query/KV head 공유 구조</sub></p>
...
&k_g&=W_g^Kx,
&v_g&=W_g^Vx,\\
o_h&=\mathrm{softmax}\!\left(\frac{q_hK_{g(h)}^{\top}}{\sqrt{d_h}}\right)V_{g(h)}.
\end{aligned}
```
...
```math
\begin{aligned}
W_g^K&=\mathrm*{mean}_{h\in\mathrm{group}\ g}W_h^K,\\
W_g^V&=\mathrm*{mean}_{h\in\mathrm{group}\ g}W_h^V.
\end{aligned}
```
