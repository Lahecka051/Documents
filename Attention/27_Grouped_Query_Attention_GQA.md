...
![MHA·GQA·MQA의 KV head grouping](https://github.com/user-attachments/assets/6d693310-6b73-429d-a758-43d6df80db3a)
...
o_h = softmax(q_h K_{g(h)}^T / sqrt(d_h)) V_{g(h)}
```
...
```text
W_g^K = mean_{h in group g} W_h^K
W_g^V = mean_{h in group g} W_h^V
```
