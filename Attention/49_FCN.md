...
```math
y_{ij}=f_{ks}\left(
\lbrace x_{si+\delta_i,sj+\delta_j}\rbrace_{0\leq\delta_i,\delta_j\leq k}
\right)
```
...
f'_{ij}=
\begin{cases}
f_{i/s,j/s}, & s\text{가 }i,j\text{를 모두 나눌 때}\\
0, & \text{그 외}
\end{cases}
```
...
```math
Y=\mathrm{ConvTranspose}(X;W,\text{stride}=f)
```
