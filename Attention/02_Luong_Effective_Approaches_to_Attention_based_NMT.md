...
세 번째 질문에는 input-feeding으로 답한다. 현재 attention 결과 `h_t_tilde`를 다음 decoder step의 입력에 다시 넣어 과거 alignment 선택을 recurrent state가 기억하게 한다.
...
Source sentence `x_1,...,x_S`를 encoder LSTM이 읽어 source hidden states를 만든다.

```text
h_bar_1, h_bar_2, ..., h_bar_S
```

Decoder는 target history를 읽어 현재 hidden state `h_t`를 만든다.

```text
h_t = LSTM(h_(t-1), input_t)
```
...
Context와 current target hidden state를 concatenate하고 projection과 `tanh`를 적용한다.

```text
h_t_tilde = tanh(W_c [c_t; h_t])
```

Output distribution은 attentional hidden state에서 계산한다.

```text
p(y_t | y_<t, x) = softmax(W_s h_t_tilde)
```

`h_t_tilde`는 decoder language-model state `h_t`와 source retrieval result `c_t`를 결합한 표현이다.
...
```text
a_t(s)
= exp(score(h_t, h_bar_s))
  / sum_s' exp(score(h_t, h_bar_s'))
```

Context는 source states의 weighted sum이다.

```text
c_t = sum_s a_t(s) h_bar_s
```
...
Content-based attention은 `h_t`와 각 `h_bar_s`를 직접 비교한다.

```text
a_t(s) = softmax_s(score(h_t, h_bar_s))
```
...
```text
score(h_t, h_bar_s) = h_t^T h_bar_s
```
...
```text
score(h_t, h_bar_s) = h_t^T W_a h_bar_s
```
...
Input-feeding은 이전 step의 `h_(t-1)_tilde`를 다음 decoder input과 concatenate한다.

```text
decoder_input_t
= concat(embedding(y_(t-1)), h_(t-1)_tilde)
```
...
```text
scores = H_enc @ h_t[:, :, None]
shape: [B, S, 1] -> [B, S]

a_t = softmax(scores, dim=source_position)
shape: [B, S]
