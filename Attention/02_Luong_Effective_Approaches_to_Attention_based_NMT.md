...
```math
h_t=\mathrm{LSTM}(h_{t-1},\mathrm{input}_t)
```
...
```math
p(y_t\mid y_{<t},x)=\mathrm{softmax}(W_s\tilde h_t)
```
...
```math
a_t(s)
=\frac{\exp(\mathrm{score}(h_t,\bar h_s))}
{\sum_{s'}\exp(\mathrm{score}(h_t,\bar h_{s'}))}
```
...
```math
a_t(s)=\mathrm{softmax}_s\!\left(\mathrm{score}(h_t,\bar h_s)\right)
```

논문은 초기 실험으로 source content를 보지 않는 location-based 형태도 둔다.

```math
a_t=\mathrm{softmax}(W_a h_t)
```
...
```math
\mathrm{score}(h_t,\bar h_s)=h_t^\top\bar h_s
```
...
```math
\mathrm{score}(h_t,\bar h_s)=h_t^\top W_a\bar h_s
```
...
```math
\begin{aligned}
\mathrm{key}_s &= W_a\bar h_s,\\
\mathrm{score} &= h_t^\top\mathrm{key}_s.
\end{aligned}
```
...
<p align="center"><sub>Figure 3 — 예측 정렬 위치 주변의 작은 창에 집중하는 Luong local attention</sub></p>

Local attention은 먼저 현재 target word가 대응할 source 중심 위치 $p_t$를 정하고, 그 주변 $[p_t-D,p_t+D]$만 본다.

```math
\begin{aligned}
\mathrm{window} &= [p_t-D,p_t+D],\\
\mathrm{window\ size} &= 2D+1.
\end{aligned}
```
...
```math
p_t=S\cdot\mathrm{sigmoid}\!\left(v_p^\top\tanh(W_ph_t)\right)
```
...
```math
\mathrm{decoder\_input}_t
=\mathrm{concat}\!\left(\mathrm{embedding}(y_{t-1}),\tilde h_{t-1}\right)
```
...
&\mathrm{shape}&:[B,S,1]\to[B,S],\\
a_t
&=\mathrm{softmax}(\mathrm{scores},
\mathrm{dim}=\mathrm{source\_position}),
&\mathrm{shape}&=[B,S],\\
c_t
