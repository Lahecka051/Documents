...
```math
\begin{aligned} I_t &\rightarrow \text{image preprocessing} \rightarrow \text{vision tower}\\ &\rightarrow H_t^{\mathrm{vis}} \rightarrow \text{projector}\\ &\rightarrow \text{LLM decoder} \rightarrow a_t\ \text{또는}\ a_{t:t+C-1}. \end{aligned}
```
...
```math
Q=XW_Q,\qquad K=XW_K,\qquad V=XW_V. \qquad\text{(1)}
```
...
```math
\mathrm{Attn}(Q,K,V) =\mathrm{Softmax}\!\left(\frac{QK^\top}{\sqrt d}\right)V. \qquad\text{(2)}
```
...
```math
K_i=\mathrm{Concat}(K_{i-1},k_{\mathrm{new}}),\qquad V_i=\mathrm{Concat}(V_{i-1},v_{\mathrm{new}}). \qquad\text{(3)}
```
...
```math
\mathrm{Sim}\!\left(P_t^{i,j},P_{t-1}^{i,j}\right) =\frac{P_t^{i,j}\cdot P_{t-1}^{i,j}} {\lVert P_t^{i,j}\rVert_2\,\lVert P_{t-1}^{i,j}\rVert_2}. \qquad\text{(4)}
```
...
```math
\mathcal P_{\mathrm{static}} =\mathrm{Top\text{-}k}\!\left( \left\{P_t^{i,j}\mid \mathrm{Sim}(P_t^{i,j},P_{t-1}^{i,j})\ge \tau\right\} \right). \qquad\text{(5)}
```
...
```math
A^l_{\mathrm{vis\text{-}text}} =A^l[:,v_{\mathrm{start}}:v_{\mathrm{end}}, t_{\mathrm{start}}:t_{\mathrm{end}}]. \qquad\text{(6)}
```
...
```math
A^l_{\mathrm{avg}}=\mathrm{Mean}_{\mathrm{heads}} \left(A^l_{\mathrm{vis\text{-}text}}\right)
```
...
```math
s_i^l =\frac{1}{hL_t} \sum_{r=1}^{h}\sum_{q\in\mathcal I_t} A^l_{r,q,i},\qquad i\in\mathcal I_v.
```
...
```math
\mathcal P_{\mathrm{task\text{-}relevant}} =\left\{P_t^{i,j}\mid S_{\mathrm{task\text{-}relevance}}[i,j]\ge\tau_{\mathrm{task}} \right\}. \qquad\text{(7)}
```
...
```math
\mathcal P_{\mathrm{reuse}} =\mathcal P_{\mathrm{static}} \setminus\mathcal P_{\mathrm{task\text{-}relevant}}. \qquad\text{(8)}
```
...
$`E^{l-1}=2.0`$, $`E^l=1.6`$이면 $`R^l=0.2`$로 20% 상대 감소다. $`E^l\gt E^{l-1}`$이면 음수다. $`E^{l-1}=0`$이면 정의되지 않는다.
...
```math
\alpha^l =\min\!\left(k\sum_{j=1}^{l}R^j,1\right). \qquad\text{(9)}
```

- $`\sum_{j=1}^{l}R^j`$는 layer 1부터 $`l`$까지 상대 entropy 감소를 누적한다.
- $`k`$는 민감도 scale이다. 여기의 $`k`$는 Eq. (5)의 token budget $`k`$와 다른 의미인데 같은 문자를 쓴다.
- `min(...,1)`은 상한만 보장한다. 누적합이 음수면 $`\alpha^l\lt 0`$이 될 수 있으므로 실제 reuse ratio라면 $`\max(0,\cdot)`$ 하한도 필요하다.
...
```math
\widetilde E^l =\frac{E^l-E_{\min}}{E_{\max}-E_{\min}+10^{-10}},\qquad \alpha^l_{\mathrm{code}}=1-\widetilde E^l
```
...
```math
\begin{aligned} K_t(i)&= \begin{cases} K_{t-1}(i), & i\in\mathcal P_{\mathrm{reuse}},\\ W_KH_t(i), & \text{otherwise}, \end{cases}\\[6pt] V_t(i)&= \begin{cases} V_{t-1}(i), & i\in\mathcal P_{\mathrm{reuse}},\\ W_VH_t(i), & \text{otherwise}. \end{cases} \end{aligned} \qquad\text{(10)}
```
...
```math
\mathrm{FLOPs}\approx4LD^2+2L^2D+2LDM. \qquad\text{(11)}
```
...
```math
\Delta\mathrm{FLOPs}_{\mathrm{layer}} \approx4L_rD^2+2L_r^2D+2L_rDM. \qquad\text{(12)}
```
...
```math
\Delta\mathrm{FLOPs}_{\mathrm{total}} \approx \left(4L_rD^2+2L_r^2D+2L_rDM\right) -\left(H^2+L_tL_vD+L^2D\right). \qquad\text{(13)}
```
...
```math
\mathrm{FLOPs}_{\mathrm{static\text{-}sim}} =N_{\mathrm{patch}}^2D_{\mathrm{patch}}\approx H^2. \qquad\text{(14)}
```
...
```math
\mathrm{FLOPs}_{\mathrm{task\text{-}filter}}\approx L_tL_vD. \qquad\text{(15)}
```
...
```math
\mathrm{FLOPs}_{\mathrm{entropy}}\approx L^2D. \qquad\text{(16)}
```
...
```math
\alpha^l=\min\!\left(k\sum_{j=1}^{l}R^j,1\right). \qquad\text{(17)}
```
...
```math
\Delta\mathrm{FLOPs}_{\mathrm{total}} =\sum_{l=1}^{\Omega}\Delta\mathrm{FLOPs}_{\mathrm{layer}}. \qquad\text{(18)}
```
...
```math
\text{cache slot index} =\text{multimodal sequence position} =\text{RoPE position id}.
```
...
```math
\text{FLOPs 감소}=\frac{1.807-1.275}{1.807}=29.44\%,\qquad \text{speedup}=1.417\times,
```

```math
\text{latency 감소}=\frac{68.22-50.59}{68.22}=25.84\%,\qquad \text{speedup}=1.348\times.
```
...
```math
\begin{aligned} f_{\mathrm{refresh}}&\approx\frac{1}{T_{\mathrm{policy}}},\\ f_{\mathrm{execute}}&\approx\frac{1}{T_{\mathrm{act}}},\\ f_{\mathrm{amortized}}&\approx\frac{C}{T_{\mathrm{policy}}+CT_{\mathrm{act}}} \end{aligned}
```
