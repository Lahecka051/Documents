...
$$
C(n)=\underbrace{4nd^2}_{Q,K,V,O\ \text{projection}}
+\underbrace{2n^2d}_{QK^\top\ \text{and}\ AV}
+\underbrace{2ndm}_{\text{FFN}}
$$
...
$$
A^{j,h}_{i,r}
=\mathrm{softmax}_{r}\!\left(
\frac{Q^{j,h}_{i,:}(K^{j,h}_{r,:})^\top}{\sqrt{d_k}}+M_{i,r}
\right).
$$
...
$$
\alpha_{\mathrm{img}}^{i,j,h}
=\sum_{r\in\mathcal I_{\mathrm{img}}}A^{j,h}_{i,r}
$$
...
$$
p(\hat y)=\prod_{i=1}^{N}p_M\!\left(\hat y_i\mid \hat y_{1\sim i-1};d;t\right).
\tag{1}
$$
...
$$
\log p(\hat y)=\sum_{i=1}^{N}\log p_M(\hat y_i\mid\hat y_{<i},d,t)
$$
...
$$
\alpha_{\mathrm{sys}}^{i,j}
+\alpha_{\mathrm{img}}^{i,j}
+\alpha_{\mathrm{ins}}^{i,j}
+\alpha_{\mathrm{out}}^{i,j}=1.
\tag{2}
$$
...
$$
\lambda_{\mathrm{sys}}^j=\sum_{i=1}^{n}\alpha_{\mathrm{sys}}^{i,j}.
\tag{3}
$$
...
$$
\bar\lambda_c^j=\frac{1}{n}\sum_{i=1}^{n}\alpha_c^{i,j},
\qquad \sum_c\bar\lambda_c^j=1.
$$
...
$$
\epsilon_{\mathrm{img}}^j
=\frac{\sum_{i=1}^{n}\alpha_{\mathrm{img}}^{i,j}}{|\mathrm{img}|}.
\tag{4}
$$
...
$$
\frac{0.03/35}{0.24/576}=2.06,
\quad
\frac{0.16/135}{0.24/576}=2.84,
\quad
\frac{0.57/150}{0.24/576}=9.12
$$

로 Figure의 2x, 3x, 9x와 맞는다. deep은

$$
\frac{0.85/35}{0.03/576}=466.3,
\quad
\frac{0.02/135}{0.03/576}=2.84,
\quad
\frac{0.10/150}{0.03/576}=12.8.
$$
...
s_r=f_{\phi}(A^{(K)})_r,
\qquad
\mathcal K_{\mathrm{img}}=\mathrm{TopK}\left(\{s_r:r\in\mathcal I_{\mathrm{img}}\},q\right),
\qquad q\approx(1-R)V.
$$

$$
\mathcal K
=\mathcal I_{\mathrm{sys}}
\cup\mathcal K_{\mathrm{img}}
\cup\mathcal I_{\mathrm{ins}}
\cup\mathcal I_{\mathrm{out}},
\qquad
H^{(K+1)}\leftarrow H^{(K+1)}[:,\mathrm{sort}(\mathcal K),:].
$$
...
$$
s_r^{\mathrm{paper}}\stackrel{?}{=}
\frac{1}{H_a n_q}\sum_h\sum_q A^{(K,h)}_{q,r}
$$
...
1-
\frac{
K\left(4nd^2+2n^2d+2ndm\right)
+(T-K)\left(4\hat n d^2+2\hat n^2d+2\hat n dm\right)
}{
T\left(4nd^2+2n^2d+2ndm\right)
}.
\tag{5}
$$
...
$$
\hat n=n_{\mathrm{text}}+(1-R)n_{\mathrm{img}}
=n-Rn_{\mathrm{img}}.
\qquad\text{[해설용 수식]}
$$
...
$$
\mathrm{Reduction}
=\frac{T-K}{T}\left(1-\frac{C(\hat n)}{C(n)}\right).
\qquad\text{[해설용 유도]}
$$
...
$$
C(612)
=64.173\mathrm{B}+3.835\mathrm{B}+86.633\mathrm{B}
=154.642\mathrm{B},
$$

$$
C(324)
=33.974\mathrm{B}+1.075\mathrm{B}+45.865\mathrm{B}
=80.914\mathrm{B}.
$$

$$
\frac{2C(612)+38C(324)}{40}=84.60\mathrm{B},
\qquad
\frac{84.60}{154.64}=54.71\%,
\qquad
\mathrm{Reduction}=45.29\%.
$$
...
s\in\mathbb R^{576},
\qquad
s_r=\frac1{H_a}\sum_h A^{(1,h)}_{631,36+r},
\quad r=0,\ldots,575.
$$
...
f_{\mathrm{refresh}}
\le
\frac{1}{t_{\mathrm{sensor}}+t_{\mathrm{vision}}+t_{\mathrm{prefill}}+t_{\mathrm{decode}}+t_{\mathrm{control}}}
$$
