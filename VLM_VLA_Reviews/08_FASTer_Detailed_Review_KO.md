...
표준 AR은 각 $c_i$가 모든 이전 code $c_{<i}$에 조건부인 next-token cross entropy다. BAR는 $B$개 token을 한 block으로 묶고, 현재 block의 각 token을 이전 block들 $C_{<j}$에만 조건부로 동시에 예측한다. block 내부 attention을 허용하는 mask와 `<BoBlk>`, `<EoBlk>` control token으로 일반 text AR와 block mode를 한 sequence 안에서 전환한다.
...
Table 10에서 256-size VQ들은 256개를 모두 activate하지만 높은 빈도 token 하나에 쏠린다. Fast+는 Bridge 1175/2048, XArm 1067/2048 active이고, FASTerVQ는 두 곳 모두 4096/4096 active다. 그러나 $>10^{-3}$ 빈도 token은 FASTer가 Bridge 162, XArm 38이므로 “모두 사용”과 “균등 사용”은 같은 말이 아니다.
...
$$
C\in\{1,\ldots,K\}^{N_c\times C_h\times C_a},\qquad
N=N_cC_hC_a.
$$
...
$$
\mathcal L=
\lVert a_{t:t+H}-\hat a_{t:t+H}\rVert_1
+\lVert\mathrm{DCT}(a_{t:t+H})-\mathrm{DCT}(\hat a_{t:t+H})\rVert_1
+\lambda\lVert z-\mathrm{sg}(z_q)\rVert_2^2.
\tag{1}
$$
...
$$
\frac{\partial}{\partial z_q}\lVert z-\mathrm{sg}(z_q)\rVert_2^2=0,
\qquad
\frac{\partial}{\partial z}\lVert z-\mathrm{sg}(z_q)\rVert_2^2=2(z-z_q).
$$
...
$$
\mathcal L_{AR}=-\sum_{i=1}^{N}\log p_\theta(c_i\mid c_{<i},I_t,s_t,x).
\tag{2}
$$
...
$$
C=\{C_1,\ldots,C_J\},\qquad
C_j=(c_{j,1},\ldots,c_{j,B}),\qquad N=JB.
$$
...
$$
\mathcal L_{BAR}=-\sum_{j=1}^{J}\sum_{i=1}^{B}
\log p_\theta(c_{j,i}\mid C_{<j},I_t,s_t,x).
\tag{3}
$$

Eq. (2)와의 핵심 차이는 같은 block의 $c_{j,<i}$가 조건식에 없다는 점이다. 현재 block의 $B$개 categorical prediction을 한 forward에서 얻어 순차 깊이를 $N$에서 약 $N/B=J$로 줄인다. 대신 같은 block 안 token의 직접적인 autoregressive dependence를 포기하거나, shared hidden interaction으로만 모델링한다.
...
$$
\mathrm{VRR}=\frac{N_{valid}}{N_{total}},\qquad
N_{valid}=\sum_{i=1}^{N_{total}}
\mathbf 1\left(\lVert a_i^{recon}-a_i^{gt}\rVert_1<\sigma\right).
\tag{4}
$$
...
- **표기 교정 범위:** 원문 대조에 따라 최근접 code 식의 제곱 표기, spacing 식의 `\qquad` 명령 오탈자, Figure 12 정점의 근사 판독값(97.7)을 바로잡았다. 표 안의 집합 원소수 표시는 열 구분자로 오인되지 않도록 `\lvert`·`\rvert`로 바꾸었고 수학적 의미는 유지했다. 작은 raster label의 원본 해상도 한계는 재작성으로 감추지 않았다.
